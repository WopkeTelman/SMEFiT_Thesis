...
class CoefficientsPlotter:
    ...
    def plot_posteriors(self, posteriors, labels, **kwargs):
        ...
        closure_line_labels = kwargs.get("closure_line_labels", None)
        bins = kwargs.get("bins", "fd")
        density = kwargs.get("density", True)

        def _pick_by_idx(value, idx):
                ...
                if show_closure_truth and closure_truth_points is not None:
                    if clr_idx < len(closure_truth_points):
                        ...
                        if closure_value is not None:
                            line_color = _pick_by_idx(closure_line_color, clr_idx)
                            line_style = _pick_by_idx(closure_line_style, clr_idx)
                            ax.axvline(
                                closure_value,
                                color=line_color,
                            ...
                            if show_closure_legend:
                                if isinstance(closure_line_labels, (list, tuple)):
                                    if clr_idx < len(closure_line_labels):
                                        legend_label = closure_line_labels[clr_idx]
                                    else:
                                        legend_label = f"{closure_line_label} {clr_idx + 1}"
                                elif closure_line_labels is not None:
                                    legend_label = str(closure_line_labels)
                                else:
                                    legend_label = closure_line_label
                                closure_legend_specs[str(legend_label)] = (
                                    line_color,
                                    line_style,
                                )
        ...
        lines = list(lines)
        labels = list(labels)
        if show_closure_truth and show_closure_legend:
            if len(closure_legend_specs) == 0 and closure_line_label not in labels:
                closure_legend_specs[str(closure_line_label)] = (
                    _pick_by_idx(closure_line_color, 0),
                    _pick_by_idx(closure_line_style, 0),
                )
            for legend_label, (line_color, line_style) in closure_legend_specs.items():
